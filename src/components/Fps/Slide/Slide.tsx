...
const Slide: React.FC<SlideProps> = ({
    ...
    return (
        <div className="flex flex-col gap-8 md:gap-0 w-full">
            <div className="flex flex-col-reverse lg:flex-row gap-6 md:gap-0 w-full items-center h-[440px] md:h-[920px]">
                    ...
                    <Suspense fallback={<LoadingSpinner />}>
                        {slide.singleImage ? (
                            <LazyImage
                                src={slide.images[0]}
                                alt={`Слайд ${slide.id}`}
                                className="w-full object-cover h-[280px] md:h-[920px]"
                            />
                        ) : (
                            <div className="grid grid-cols-2">
                                {slide.images.map((imgSrc, index) => (
                                    <div
                                        key={index}
                                        className={`rounded-lg overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-green-500/30 ${isTransitioning ? 'opacity-70 scale-95' : 'opacity-100 scale-100'}`}
                                        style={{ marginTop: '10px', marginRight: (index % 2 === 0) ? '10px' : '0' }}
                                    >
                                        <LazyImage
                                            src={imgSrc}
